...
export const thunderous_smite_expanded = 'Вперше, коли ви потрапляєте атакою ближньою зброєю, поки закляття активне, ваша зброя видає громовий гуркіт, чутний з відстані 300 футів, і атака завдає цілі додаткове пошкодження Звуком 2d6.<br/><br/>' +
...
'* Ви можете один раз створити закляття Пристріт, використовуючи слот мани. Ви не можете зробити це повторно, поки не закінчите тривалий відпочинок.';
...
export const polymorph_warlock_expanded = polymorph_expanded+'<br/><br/>* Ви можете створити це закляття один раз. Ви не можете зробити це повторно, доки не закінчите тривалий відпочинок.';
...
export const confusion_warlock_expanded = confusion_expanded+'<br/><br/>* Ви можете створити це закляття один раз. Ви не можете зробити це повторно, доки не закінчите тривалий відпочинок.';
...
'* Ви можете один раз створити закляття, використовуючи слот мани. Ви не можете зробити це повторно, поки не закінчите тривалий відпочинок.';
...
export const stinking_cloud_expanded = 'Ви створюєте сферу з радіусом 20 футів жовтого нудотворного газу із центром на точці в межах дистанції. Ця хмара обходить кути, і її простір — сильно затулена місцевість. Хмара тримається у повітрі, доки активно закляття.<br/><br/>'+
...
export const passwall_expanded = 'У точці, яку ви бачите в межах дистанції на деревʼяній, оштукатуреній або камʼяній поверхні (такі як стіна, стеля або підлога) зʼявляється прохід, що існує, поки активно закляття. Ви самі визначаєте габарити проходу: до 5 футів завширшки, до 8 футів заввишки і до 20 футів завглибшки. Цей прохід не дестабілізує саму споруду. Коли прохід зникає, всі істоти та предмети, що все ще знаходяться в ньому, безпечно виштовхуються у вільний простір, найближчий до поверхні, на якій ви активували це закляття.<br/><br/>'+
...
'Ви або згодна істота, якої ви торкаєтеся, входите в транс, стаючи посильним. Перебуваючи у трансі, посильний усвідомлює своє оточення, але не може здійснювати дії і переміщатися. Якщо ціль спить, посильний зʼявляється у її сні і може розмовляти з нею, поки та спить, і поки що закляття активно. Посильний може формувати оточення сну, створюючи місцевість, предмети та інші образи. Посильний може будь-коли вийти з трансу, закінчивши ефект цього закляття. Після пробудження ціль чудово памʼятає сон. Ви можете зробити посильного кошмарним для мети. У цьому випадку посильний може передати послання не довше десяти слів, і ціль зобовʼязана здійснити Випробування Мудрості.<br/><br/>'+
...
export const conjure_elemental_expanded = 'Ви викликаєте елементаля, що слухаєьться вас. Виберіть область води, повітря, землі або вогню, що уміститься в куб з довжиною ребра 10 футів. У вільному просторі в межах 50 футів від обраної області зʼявляється відповідний місцевості елементаль з показником небезпеки 5 або менше. Наприклад, з багаття виходить Вогненний елементаль, а Земляний елементаль вибирається з-під землі. Елементаль зникає, коли його здоровʼя опускається до 0 або коли закляття закінчується. Елементаль дружній до вас і ваших супутників, поки закляття активно.<br/><br/>'+

'Здійсніть перевірку ініціативи за елементаля, і він здійснюватиме свої власні ходи. Він підпорядковується відданим вами усно командам (дія не потрібна). Якщо ви не віддаєте команду, він захищається від ворожих істот, але дій не робить. Якщо ваша концентрація порушена, елементаль не зникає. Натомість ви втрачаєте над ним контроль, і він стає ворожим до вас і ваших супутників, і може напасти. Некерований елементаль не може бути відпущений вами, але він зникає через 1 годину після того, як був викликаний.<br/><br/>'+
...
export const conjure_elemental_warlock_expanded = conjure_elemental_expanded+'<br/><br/>* Ви можете один раз створити закляття, використовуючи слот мани. Ви не можете зробити це повторно, поки не закінчите тривалий відпочинок.';
...
export const cloudkill_expanded = 'Ви створюєте сферу з радіусом 20 футів із отруйного жовто-зеленого туману з центром на точці, обраній вами в межах дистанції. Цей туман огинає кути. Він існує, поки активно закляття, або поки сильний вітер його не розвіє. Його простір — сильно загороджена місцевість. Коли істота вперше за хід входить в область закляття або починає там хід, вона повинна пройти Випробування Статури. Істота отримує пошкодження Отрутою 5d8 при провалі або половину цього пошкодження у разі успіху. Істота підпадає під дію закляття навіть якщо затримує дихання або взагалі не має дихати. Хмара на початку кожного вашого ходу переміщається на 10 футів від вас, котячись по поверхні. Випари, будучи важчими за повітря, стікаються вниз, і навіть можуть проникати в отвори.<br/><br/>'+
...
export const bestow_curse_expanded = 'Ви торкаєтеся істоти, і воно має досягти успіху в спаски Мудрості, інакше стане проклятим на період дії заклинання. Накладаючи це заклинання, виберіть характер прокляття з наступного списку:<br/><br/>'+
...
'• Поки мета проклята, ваші атаки та заклинання завдають їй додаткової шкоди некротичною енергією 1d8. Заклинання Зняття прокляття закінчує цей ефект.<br/><br/>'+
...
'Якщо ви накладаєте це заклинання, використовуючи комірку 4 або вище рівня, тривалість буде «Концентрація, аж до 10 хвилин». Якщо ви використовували комірку 5 або вище рівня, тривалість буде «8 годин». Якщо ви використовували комірку 7 або вище рівня, тривалість буде «24 години». Якщо ви використовували комірку 9 рівня, заклинання діє, поки не розсіється. При використанні комірки щонайменше 5 рівня концентрація не потрібна.';

export const bestow_curse_warlock = bestow_curse + ' *';

export const bestow_curse_warlock_details = bestow_curse_details;

export const bestow_curse_warlock_expanded = bestow_curse_expanded+'<br/><br/>* Ви можете один раз створити закляття, використовуючи слот мани. Ви не можете зробити це повторно, поки не закінчите тривалий відпочинок.';