...
export default {
	setup() {
		...
		return { MY, main_page, dic, showSettings__Main };
	},
	computed: {
		Lang_Icon: store =>
			require(`@/assets/img/icon/lang/icon_${store.dic.select_lang}.png`),

		Em_Icon: store =>
			store.dic.lang.find(icon => icon.mark === store.dic.select_lang).icon,

		Char_Lvl: store => `lvl ${store.MY.level}`,
	},
};
</script>
